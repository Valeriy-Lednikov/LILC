...
struct PtrHash { size_t operator()(Id p) const noexcept { return reinterpret_cast<size_t>(p); } };
struct PtrEq   { bool   operator()(Id a, Id b) const noexcept { return a == b; } };
...
private:
    ...
    struct VarChange { Id id; double* prev; };
    struct ArrChange { Id id; std::vector<double>* prev; };
...
public:
    ...
    void inLevel() {
        ++currentLevel;
        if ((int)varFrames.size() <= currentLevel) varFrames.emplace_back();
        if ((int)arrFrames.size() <= currentLevel) arrFrames.emplace_back();
    ...
    void outLevel() {
        ...
        while (arrLog.size() > aMark) {
            auto ch = arrLog.back(); arrLog.pop_back();
            if (ch.prev == nullptr) liveArrays.erase(ch.id);
            else liveArrays[ch.id] = ch.prev;
        }
        arrMarks.pop_back();
        ...
        while (varLog.size() > vMark) {
            auto ch = varLog.back(); varLog.pop_back();
            if (ch.prev == nullptr) liveVars.erase(ch.id);
            else liveVars[ch.id] = ch.prev;
        }
        varMarks.pop_back();
    ...
    void addVar(Id name, double value) {
        ...
        if (!inserted) it->second = value; // переопределили на уровне

        double* prev = nullptr;
        if (auto itLive = liveVars.find(name); itLive != liveVars.end()) prev = itLive->second;
    ...
    bool setVar(Id name, double value) {
        ...
        if (it == liveVars.end()) return false;
        *it->second = value;
        return true;
    }
    ...
    void addArray(Id name, size_t size, double init = 0.0) {
        ...
        if (!inserted) it->second.assign(size, init);

        std::vector<double>* prev = nullptr;
        if (auto itLive = liveArrays.find(name); itLive != liveArrays.end()) prev = itLive->second;
    ...
    bool setArrayElem(Id name, size_t index, double value) {
        ...
        if (it == liveArrays.end()) return false;
        auto& vec = *it->second;
        if (index >= vec.size()) {
    ...
    const std::vector<variable>& getVarsAtLevel(int level) const {
        ...
        tmp.clear();
        for (const auto& kv : varFrames[level]) tmp.emplace_back(kv.first, kv.second);
        return tmp;
    }
    ...
    const std::vector<array_var>& getArraysAtLevel(int level) const {
        ...
        tmp.clear();
        for (const auto& kv : arrFrames[level]) tmp.emplace_back(kv.first, kv.second);
        return tmp;
    }
    ...
    void reserve(size_t varsPerLevel, size_t arraysPerLevel, size_t levels = 8) {
        ...
        for (auto& f : varFrames) f.reserve(varsPerLevel);
        for (auto& f : arrFrames) f.reserve(arraysPerLevel);
